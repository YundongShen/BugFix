...
classDiagram
    ...
    class Index {
        ...
        + createIndex(data: object) -> void
        + queryIndex(query: string) -> array
    }
    class Ranking {
        + rankResults(results: array) -> array
    }
    class Summary {
        + summarizeResults(results: array) -> string
    }
    class DataSource {
    ...
    UserInterface --> Controller : Trigger
    Controller --> SearchLogic : Invoke
    SearchLogic --> Index : Use
    SearchLogic --> Ranking : Use
    SearchLogic --> Summary : Use
    Index --> DataSource : Access
    
    %% Optional call relationships for different scenarios
    class ApiGateway {
        + receiveRequest(query: string) -> string
    }
    class HttpClient {
        + sendRequest(query: string) -> string
    }

    UserInterface --> HttpClient : Optionally for Front - end
    HttpClient --> ApiGateway : Optionally for Front - end
    ApiGateway --> Controller : Optionally for Back - end
"""