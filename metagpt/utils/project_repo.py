...
from metagpt.const import (
    ...
    FRONTEND_DOCKERFILE_REPO,
    BACKEND_DOCKERFILE_REPO
)
...
class DocFileRepositories(FileRepository):
    ...
    testcase: FileRepository
    code_design: FileRepository
    task: FileRepository
    code_summary: FileRepository
    graph_repo: FileRepository
    class_view: FileRepository
    code_plan_and_change: FileRepository
    frontend_dockerfile_repo: FileRepository
    backend_dockerfile_repo: FileRepository

    def __init__(self, git_repo, local: bool = False):
        super().__init__(git_repo=git_repo, relative_path=DOCS_FILE_REPO, local=local)
        ...
        self.testcase = git_repo.new_file_repository(relative_path=TESTCASE_FILE_REPO)
        self.code_design = git_repo.new_file_repository(relative_path=CODE_DESIGN_FILE_REPO)
        ...
        self.frontend_dockerfile_repo =git_repo.new_file_repository(relative_path=FRONTEND_DOCKERFILE_REPO)
        self.backend_dockerfile_repo = git_repo.new_file_repository(relative_path=BACKEND_DOCKERFILE_REPO)
...
class ResourceFileRepositories(FileRepository):
    ...
    def __init__(self, git_repo, local: bool=False):
        super().__init__(git_repo=git_repo, relative_path=RESOURCES_FILE_REPO, local=local)
...
class ProjectRepo(FileRepository):
    def __init__(self, root: str | Path | GitRepository, local: bool = False):
        ...
        super().__init__(git_repo=git_repo_, relative_path=Path("."), local=local)
        self._git_repo = git_repo_
        self.docs = DocFileRepositories(self._git_repo, local=local)
        self.resources = ResourceFileRepositories(self._git_repo, local=local)
