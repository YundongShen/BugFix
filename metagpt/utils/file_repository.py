...
from metagpt.pipe_files import get_pipein,read_pipe_message_with_len
from eazytec_send import set_doc_for_recv

def extract_file_path(sid: None, file_path: str) -> str:

    worksps_pos  = file_path.find("workspace")
    #print(f"#########extract file sid: {sid}")

    remain_str = ""
    if worksps_pos >=0:
        remain_str = "/workspace"+file_path[worksps_pos+len("workspace"):]
        parts = remain_str.split("/")
        if sid in parts:
            # 找到要删除的部分的索引并删除
            index_to_remove = parts.index(sid)
            del parts[index_to_remove]
            remain_str = "/".join(parts)
        print(f"####### extract path: {remain_str}")
...
class FileRepository:
    ...
    def __init__(self, git_repo, relative_path: Path = Path("."), local: bool = False):
    ...
    async def save(self, filename: Path | str, content, dependencies: List[str] = None, current_role: str="", role_task: str="", sid: str="", user_intend: str="") -> Document:
        ...
        logger.debug(f"#####save pathname: {pathname}, path parent: {pathname.parent}")
        ...
        oh_action_data = {}
        oh_action_data['message'] = f"save to: "+ extract_file_path(self._git_repo.sid, str(pathname))
        oh_action_data['action_type'] = "FILE_SAVE"
        oh_action_data['file_path'] = extract_file_path(sid, str(pathname))
        oh_action_data['file_content'] = content
        oh_action_data['conversation_id'] = sid
        logger.error(f"<OH_ACTION> json_data:{json.dumps(oh_action_data)}")

        if((role_task=="") or (sid == "") or (current_role == "") or (user_intend=="")):
            print(f"#########file save erorr.")

        content_info = {
            "sub_content": f"Finish writing {filename}",
            "role_task": role_task,
    ...
    async def check_oh_runtime_file_exists(self, filename: Path | str) -> str:
        ###Check if a file exists in the OH runtime environment.
        path_name = self.workdir / filename
        oh_action_data = {}
        oh_action_data['message'] = f"check: "+ extract_file_path(self._git_repo.sid, str(path_name)) + " exists or not or file or dir"
        oh_action_data['action_type'] = "CHECK_FILE_EXISTS"
        oh_action_data['file_path'] = extract_file_path(self._git_repo.sid, str(path_name))
        #oh_action_data['file_path'] = "/workspace/"+str(filename)
        # logger.error(f"<OH_ACTION> json_data:{json.dumps(oh_action_data)}")

        file_str = read_pipe_message_with_len(get_pipein())
        file_str = file_str.strip()

        if file_str == "NOTFOUND":
            return "NOTFOUND"
        elif file_str == "DIR":
            return "DIR"
        else:
    ...
    async def get(self, filename: Path | str) -> Document | None:
        ...
        path_name = self.workdir / filename
        #if await self.check_oh_runtime_file_exists(filename) != "FILE":
            #return None
        ...
        if self.local:
            ...
            oh_action_data = {}
            oh_action_data['message'] = f"read: "+ extract_file_path(self._git_repo.sid, str(path_name))
            oh_action_data['action_type'] = "FILE_READ"
            oh_action_data['file_path'] = extract_file_path(self._git_repo.sid, str(path_name))
            oh_action_data['conversation_id'] = self._git_repo.sid
            logger.error(f"<OH_ACTION> json_data:{json.dumps(oh_action_data)}")

            set_doc_for_recv(self._git_repo.sio, self._git_repo.sid, doc)
    ...
    async def save_doc(self, doc: Document, dependencies: List[str] = None, current_role: str="", role_task: str="", sid: str="", user_intend: str=""):
        """Save content to a file and update its dependencies.
        ...
        await self.save(filename=doc.filename, content=doc.content, dependencies=dependencies, current_role=current_role, role_task=role_task, sid=sid, user_intend=user_intend)
        # logger.debug(f"File Saved: {str(doc.filename)}")

    async def save_pdf(self, doc: Document, with_suffix: str = ".md", dependencies: List[str] = None, current_role: str="", role_task: str="", sid: str="", user_intend: str=""):
        ...
        await self.save(filename=str(filename), content=json_to_markdown(m), dependencies=dependencies, current_role = current_role, role_task=role_task, sid=sid, user_intend=user_intend)
