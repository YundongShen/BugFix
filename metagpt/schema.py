...
class CodingContext(BaseContext):
    filename: str
    fix_file_desc: Optional[str] = None
    prd_doc: Optional[Document] = None
    testcase_doc: Optional[Document] = None
    design_doc: Optional[Document] = None
    task_doc: Optional[Document] = None
    code_doc: Optional[Document] = None
    playwright_code_doc: Optional[Document] = None
    code_plan_and_change_doc: Optional[Document] = None
    bugfix_doc: Optional[Document] = None
    file_list: Optional[List] = []
    bugfile_list: Optional[List] = []




class SystemTestingContext(BaseContext):
    filename: str
    prd: str
    html_contents: str
    sys_test_doc: Optional[Document] = None

class SystemTestingPlayWrightCodeContext(BaseContext):
    filename: str
    prd: str
    design: str
    frontend_code: str
    sys_test_doc: str
    sys_test_playwright_code:Optional[Document] = None

class LaunchProjectTestingContext(BaseContext):
    filename: str
    project_path: str
    project_src_path:str
    launch_front_file: str
    launch_backend_file: str
    launch_requirements_file: str
    front_dependency: bool
    backend_dependency: bool
    launch_success: bool
    launch_front_success: bool
    build_front_success: bool
    file_list: list
    launch_result_analysis: Optional[Document] = None

class ProjectIntegrationTestingContext(BaseContext):
    filename: str
    project_path: str
    project_src_path:str
    launch_front_file: str
    launch_backend_file: str
    integration_test_case_file_path: str
    test_success: bool
    file_list: list
    integration_test_result: Optional[Document] = None

class DockerfileContext(BaseContext):
    launch_frontend_file: str
    launch_backend_file: str
    launch_requirements_file: str
    launch_frontend_command: str = "npm run dev"
    launch_backend_command: str = "uvicorn main:app --port $APP_PROT_2"
    install_frontend_command: str = "npm install --registry=http://2.2.0.23:45923/repository/group-npm/"
    install_backend_command: str = "pip install --no-cache-dir -r requirements.txt -i https://mirrors.tuna.tsinghua.edu.cn/pypi/web/simple"
    frontend_image: str = "harbor.eazytec-building.com/eazytec/node:22.12.0"
    backend_image: str = "hb.eazytec-cloud.com/eazydevelop/python:3.12.3-slim-amd64"
    ...
    def loads(filenames: List, **kwargs) -> CodePlanAndChangeContext:
        ...
        for filename in filenames:
            ...
            if filename.is_relative_to(SYSTEM_DESIGN_FILE_REPO):
                ...
                continue
            if filename.is_relative_to(CODE_DESIGN_FILE_REPO):
                ctx.code_design_filename = filename.name
