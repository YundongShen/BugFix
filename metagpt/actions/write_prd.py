...
class WritePRD(Action):
    ...
    async def run(self, with_messages, *args, **kwargs) -> ActionOutput | Message:
        ...
        if related_docs := await self.get_related_docs(req, docs):
            ...
            self.config.role_task = f"2/2 WritePRD"

            oh_action_data = {}
            content_info = {
                "sub_content": f"New requirement detected: {req.content}",
    ...
    async def _handle_bugfix(self, req: Document) -> Message:
        # ... bugfix logic ...
        await self.repo.docs.save(filename=BUGFIX_FILENAME, content=req.content, current_role=self.config.current_role, role_task=self.config.role_task,sid = self.config.sid, user_intend=self.config.user_intend)
        await self.repo.docs.save(filename=REQUIREMENT_FILENAME, content="", current_role=self.config.current_role, role_task=self.config.role_task,sid = self.config.sid, user_intend=self.config.user_intend)
    ...
    async def _handle_new_requirement(self, req: Document) -> ActionOutput:
        ...
        if self.config.simulate:
            prd_path = SIMULATE_ROOT / PRD_FILENAME
            with open(prd_path, encoding="utf-8") as f:
                content =f.read()
            new_prd_doc = await self.repo.docs.prd.save(
                filename=PRD_FILENAME, content=content,
                current_role = self.config.current_role, role_task = self.config.role_task,
                sid =self.config.sid, user_intend=self.config.user_intend
            )
        else:
            new_prd_doc = await self.repo.docs.prd.save(
                filename=PRD_FILENAME, content=node.instruct_content.model_dump_json(),
                current_role = self.config.current_role, role_task = self.config.role_task,
                sid =self.config.sid, user_intend=self.config.user_intend
            )
        await self._save_competitive_analysis(new_prd_doc)
        await self.repo.resources.prd.save_pdf(doc=new_prd_doc, current_role=self.config.current_role, role_task=self.config.role_task, sid = self.config.sid, user_intend=self.config.user_intend)
    ...
    async def _update_prd(self, req: Document, prd_doc: Document) -> Document:
        ...
        await self.repo.docs.prd.save_doc(doc=new_prd_doc, current_role=self.config.current_role, role_task=self.config.role_task,sid = self.config.sid, user_intend=self.config.user_intend)
        await self._save_competitive_analysis(new_prd_doc)
        await self.repo.resources.prd.save_pdf(doc=new_prd_doc, current_role=self.config.current_role, role_task=self.config.role_task,sid = self.config.sid, user_intend=self.config.user_intend)
        return new_prd_doc
