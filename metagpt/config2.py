...
from metagpt.configs.project_config import ProjectConfig
...
class Config(CLIParams, YamlModel):
    ...
    sid :str = ""
    sio: Any = Field(default=None, exclude=True)
    local: bool = False
    simulate: bool = True
    current_role: str=""
    role_task: str=""
    ...
    @classmethod
    def default(cls, sid: str="", sio=None, local: bool = False, idea: str="", llm_config : dict=None, simulate: bool = True,):
        """Load default config
        ...
        default_config_paths: List[Path] = [
            ...
            METAGPT_ROOT / "config/project.yaml",
        ]

        dicts = [dict(os.environ)]
        dicts += [Config.read_yaml(path) for path in default_config_paths]

        dicts.append({"sid": sid})
        dicts.append({"local": local})            
        dicts.append({"user_intend": idea})
        dicts.append({"simulate": simulate})

        if sio:
            dicts.append({"sio": sio})

        if ((local == False) and (llm_config)):
            provider = llm_config["model"].split("/")
            dicts[1]["llm"]["api_type"] = provider[0]
            dicts[1]["llm"]["model"] = provider[1]
            dicts[1]["llm"]["base_url"] = llm_config["base_url"]
            dicts[1]["llm"]["api_key"] = llm_config["api_key"]
