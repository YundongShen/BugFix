...
class QaEngineer(Role):
    ...
    async def _write_test(self, message: Message) -> None:
        ...
        for filename in changed_files:
            ...
            await self.project_repo.tests.save_doc(
                ...
                current_role=self.config.current_role, role_task=self.config.role_task, sid = self.config.sid, user_intend=self.config.user_intend
            )
    ...
    async def _debug_error(self, msg):
        ...
        await self.project_repo.tests.save(filename=run_code_context.test_filename, content=code, current_role=self.config.current_role, role_task=self.config.role_task, sid = self.config.sid, user_intend=self.config.user_intend)
