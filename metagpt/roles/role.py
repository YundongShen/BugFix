...
class Role(SerializationMixin, ContextMixin, BaseModel):
    ...
    def project_repo(self) -> ProjectRepo:
        project_repo = ProjectRepo(self.context.git_repo, self.config.local)
    ...
    def set_env(self, env: "Environment"):
        ...
        if env:
            ...
            self.llm.system_prompt = self._get_prefix()
            self.llm.cost_manager = self.context.cost_manager
            self.rc.sid = self.context.config.sid
            self.rc.sio = self.context.config.sio
    ...
    async def run(self, with_message=None) -> Message | None:
        ...
        self.use_flag = True
        self.cause_action = rsp.cause_by
